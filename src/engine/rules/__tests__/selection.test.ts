...
describe('dated rule selection', () => {
  ...
  it('links each pinned field to dated official evidence and records MB conflict', () => {
    ...
    expect(pe.sourceConflict).toMatch(/January CRA.*142,250.*17\.62%.*19%/s)
    expect(pe.sourceConflict).toMatch(/government.*142,520.*200,000.*20%/s)
    expect(pe.sourceConflict).not.toMatch(/neither|absent|unsupported/i)
    expect(pe.fieldSources.provincialBrackets).toContain('/2026/t4032-pe-1-26e.pdf')

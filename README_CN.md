...
**税务引擎**：真实的联邦 + 省级边际税率表（ON / QC / BC / AB，2025–26 数值，
数据驱动、每年更新）、基本免税额、魁省联邦税减免、按 ACB 追踪的资本利得 50% 计入、
按人计算的 OAS 回收税，以及夫妻模式下的收入分割计税。
...
按工作经历估算 CPP/QPP（最好 39 年规则——提前退休会稀释平均值）、按居住年数估算
OAS、CPP 60–70 / OAS 65–70 逐岁时机表格、以及蒙特卡洛模拟（Web Worker 里跑
1000 次随机收益）附失败解剖。
...
- 税务数据：2025–26 联邦 + ON/QC/BC/AB 四省税表，每年手动更新。
- 夫妻计税假设理想 50/50 收入分割。现实中 65 岁前 RRSP 取款只计入账户所有人——
  桥接期要平分，前提是双方 RRSP 余额相当（提前用 spousal RRSP 布局）。
- 尚未建模：非注册账户利息/股息的年度税拖累（这会*低估* meltdown 的现实优势）、
  安省 surtax、股息税收抵免、TFSA/RRSP 供款上限、其余省份。
- 蒙特卡洛采用正态分布年收益，未建模资产间相关性。
